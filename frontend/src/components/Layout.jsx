...
export default function Layout(){
 ...
 return <>
  <header className="header"><div className="headerInner"><Brand/><nav className="desktopNav"><NavLink to="/">Beranda</NavLink><NavLink to="/menu">Menu</NavLink><NavLink to="/kategori">Kategori</NavLink><NavLink to="/promo">Promo</NavLink></nav><div className="headerSearch"><Search size={18}/><input placeholder="Cari menu JJ Twins mart..." onKeyDown={e=>{if(e.key==='Enter'&&e.currentTarget.value) location.href=`/menu?q=${encodeURIComponent(e.currentTarget.value)}`}}/></div><div className="headerActions"><Link className="cartHeaderBtn" to="/keranjang" aria-label={`Keranjang, ${count} item`}><ShoppingCart size={20}/>{count>0&&<b>{count}</b>}</Link><button className="themeToggle" type="button" aria-label={isDark?'Gunakan mode terang':'Gunakan mode gelap'} title={isDark?'Mode terang':'Mode gelap'} aria-pressed={isDark} onClick={toggleTheme}>{isDark?<Sun size={20}/>:<Moon size={20}/>}</button></div></div><div className="mobileSearch"><Search size={18}/><input placeholder="Cari beras, minyak, minuman..." onKeyDown={e=>{if(e.key==='Enter'&&e.currentTarget.value) location.href=`/menu?q=${encodeURIComponent(e.currentTarget.value)}`}}/></div></header>
  <main><Outlet/></main>
