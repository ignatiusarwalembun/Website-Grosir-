...
export function ProductCard({p}){const {add}=useCart();const [added,setAdded]=useState(false); return <article className="productCard">
 <Link className="productVisual" to={`/produk/${p.id}`}><span>{p.emoji}</span>{p.discount>0&&<b>-{p.discount}%</b>}</Link>
 <div className="productMeta"><small>{p.tag}</small><Link className="productName" to={`/produk/${p.id}`}>{p.name}</Link><span className="unit">{p.unit}</span><del>{rupiah(p.price)}</del><strong>{rupiah(p.wholesalePrice)}</strong><p><CheckCircle2 size={14}/> Tersedia · Min. {p.minOrder} pcs</p><div className="tierHint"><Tag size={14}/>{rupiah(p.wholesalePrice)} / pcs jika beli ≥10</div></div>
 <button className={added?'addBtn added':'addBtn'} onClick={()=>{add(p,p.minOrder);setAdded(true);setTimeout(()=>setAdded(false),900)}}><Plus size={18}/>{added?'Ditambahkan':'Keranjang'}</button>
 </article>}
